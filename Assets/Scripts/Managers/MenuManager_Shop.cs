...
public sealed partial class MenuManager : MonoBehaviour
{
    ...
    int skill1GreedLevel, skill2ShieldLevel;
    ...
    // Update cosmetic item text colors, preview image, and buy button visibility
    ...
    // Purchase selected cosmetic if player has enough coins
    ...
    // Open skill shop and refresh skill UI
    ...
    // Purchase a skill upgrade if player has enough coins
    public void BuySkill(int index)
    {
        if (coin >= SkillUnlockCost)
        {
            ...
            if (index == 0) skill1GreedLevel++;
            else if (index == 1) skill2ShieldLevel++;
            UpdateSkillUI();
        }
        else audioSource.PlayOneShot(cantDoThatSound);
    }

    // Update skill level labels and buy button visibility
    void UpdateSkillUI()
    {
        skillLevelTexts[0].text = skill1GreedLevel + " / 3";
        skillLevelTexts[1].text = skill2ShieldLevel + " / 3";
        skillBuyButtons[0].SetActive(skill1GreedLevel < 3);
        skillBuyButtons[1].SetActive(skill2ShieldLevel < 3);
